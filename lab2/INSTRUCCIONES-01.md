...
Instrucciones para investigar, documentar y remediar incidentes en el namespace `astroshop-01` en el ambiente dynatrace-lab.

## Reglas críticas

- IMPORTANT: filtra SIEMPRE por namespace `matchesPhrase(k8s.namespace.name, "astroshop-01")` en cada consulta DQL. Es tu identificador principal, viene en toda la telemetría y es único para ti. Sin este filtro verías datos de otros participantes.
...
- Al abrir el PR, nombra el branch SIEMPRE `fix/payment-01-off`. No reutilices branches existentes; si existe, créalo desde main actualizado.
