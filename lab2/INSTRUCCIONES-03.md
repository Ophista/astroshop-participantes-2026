...
Instrucciones para investigar, documentar y remediar incidentes en el namespace `astroshop-03` en el ambiente dynatrace-lab.

## Reglas críticas

- IMPORTANT: filtra SIEMPRE por namespace `matchesPhrase(k8s.namespace.name, "astroshop-03")` en cada consulta DQL. Es tu identificador principal, viene en toda la telemetría y es único para ti. Sin este filtro verías datos de otros participantes.
...
- Proponer fixes → usa el MCP de GitHub. Abre un Pull Request sobre `flags/payment-03.yaml` en `leidyruizrr/astroshop-participantes-2026`.
...
- Al abrir el PR, nombra el branch SIEMPRE `fix/payment-03-off`. No reutilices branches existentes; si existe, créalo desde main actualizado.
...
- Kubernetes en GCP (GKE), cluster `dt-lab-lcrr-demo`, namespace `astroshop-03`. No tienes acceso a `kubectl` ni al cluster k8s. La única vía para cambiar su estado es un Pull Request.
