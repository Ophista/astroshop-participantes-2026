# AstroShop — Investigación y remediación de incidentes (Participante 14)

Instrucciones para investigar, documentar y remediar incidentes en el namespace `astroshop-14` en el ambiente dynatrace-lab.

## Reglas críticas

- IMPORTANT: filtra SIEMPRE por namespace `matchesPhrase(k8s.namespace.name, "astroshop-14")` en cada consulta DQL. Es tu identificador principal, viene en toda la telemetría y es único para ti. Sin este filtro verías datos de otros participantes.
...
- Proponer fixes → usa el MCP de GitHub. Abre un Pull Request sobre `flags/payment-14.yaml` en `leidyruizrr/astroshop-participantes-2026`.
- En una tarea de varios pasos, usa la herramienta correcta en cada uno y dilo en tu respuesta.
- IMPORTANT: al crear cualquier recurso (notebook, SLO, dashboard), antepón SIEMPRE tu namespace al nombre: `astroshop-14 - <nombre>`. Nunca crees un recurso sin ese prefijo, para no colisionar con otros.
- Para abrir el PR usa ÚNICAMENTE la herramienta del MCP de GitHub. No uses `gh` ni git por terminal.
- Al abrir el PR, nombra el branch SIEMPRE `fix/payment-14-off`. No reutilices branches existentes; si existe, créalo desde main actualizado.
...
- Kubernetes en GCP (GKE), cluster `dt-lab-lcrr-demo`, namespace `astroshop-14`. No tienes acceso a `kubectl` ni al cluster k8s. La única vía para cambiar su estado es un Pull Request.
