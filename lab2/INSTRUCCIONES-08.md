...
Instrucciones para investigar, documentar y remediar incidentes en el namespace `astroshop-08`.
...
- Kubernetes en GCP (GKE), cluster `dt-lab-lcrr-demo`, namespace `astroshop-08`.
...
El ambiente usa flagd (OpenFeature) para inyectar fallas. El flag principal es `paymentFailure`, con estado en `flags/payment-08.yaml` (`on` = falla, `off` = sano).
