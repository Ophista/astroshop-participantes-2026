...
Instrucciones para investigar, documentar y remediar incidentes en el namespace `astroshop-09`.
...
- Kubernetes en GCP (GKE), cluster `dt-lab-lcrr-demo`, namespace `astroshop-09`.
...
El ambiente usa flagd (OpenFeature) para inyectar fallas. El flag principal es `paymentFailure`, con estado en `flags/payment-09.yaml` (`on` = falla, `off` = sano).
